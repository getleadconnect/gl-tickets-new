			...
			<div class="page-title-box">
				<div class="float-end">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active"><a href="#">Customer Tickets</a>
						</li><!--end nav-item-->
					</ol>
				</div>
				<h4 class="page-title">Customer Tickets Report </h4>
...
var table = $('#datatable1').DataTable({
		...
		ajax:
		{
			url:"{{url('view-customer-tickets-report')}}",
			data: function (data) 
		    {
               //data.search = $('input[type="search"]').val();
			   data.customer_id = $('#flt_customer_id').val();
			   data.tk_status = $('#flt_status').val();
...
$("#export_to_excel").click(function()
{
	...
	var custid=($("#flt_customer_id").val())?$("#flt_customer_id").val():0;
	var lnk="{{url('export-customer-tickets/:sdate/:edate/:custid')}}";
	var link=lnk.replace(":sdate",sdat).replace(":edate",edat).replace(":custid",custid);
